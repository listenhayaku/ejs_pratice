...
var communicator = exports.communicator = function(ip,port,data){
  return new Promise((reslove,rejects)=>{     
    ...
    pyProcess.on("close",(code)=>{
      var sql; 
      if(code == 1){
        sql = "UPDATE project.drone_info SET status="+0+" WHERE ip=\""+ip+"\" and port="+port+";";
        get_mysql(sql,()=>{});
      }
      else if(code == 0){
        sql = "UPDATE project.drone_info SET status="+1+" WHERE ip=\""+ip+"\" and port="+port+";";
        get_mysql(sql,()=>{});
      }
      else{
...
exports.monitor = function(){
  var sql = "SELECT * from project.drone_info;";
