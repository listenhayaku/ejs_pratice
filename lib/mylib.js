  ...
  return new Promise((reslove,rejects)=>{ 
    ...
    pyProcess.stdout.on("data",(data) =>{
        ...
        if(data.toString().includes("[communicator.py]successful\n")){
          ...
          //console.log("(deubg)[mylib][communicator]successful");
          for(var i = 0;i < connecting.length;i++){
            //console.log(`\tconnecting[${i}]:`,connecting[i].ip,connecting[i].port);
          }
          reslove(pyProcess);
        }
          ...
          try{
            ...
            var ip = dataline.toString().split("//")[1].split(":")[0];
            var port = dataline.toString().split("//")[1].split(":")[1];
            console.log("(debug)[mylib][communicator]ip:",ip,"port:",port);
            
            async function test(){
              var sql = "select * from project.drone_info where ip=\""+ip+"\"&& port="+port+";";
    ...
    pyProcess.on("close",(code)=>{
      ...
      //console.log("(debug)[mylib][communicator]after remove connecting");
      for(var i = 0;i < connecting.length;i++){
        //console.log(`\tconnecting[${i}]:`,connecting[i].ip,connecting[i].port);
      }
      //移除已下線的節點
